...
const searchContentEndpoint = makeEndpoint({
  ...
  path: '/search',
  alias: 'searchContent',
  description: 'Searches across all supported content types.',
  parameters: [
    { name: 'terms', type: 'Query', schema: z.string().optional() },
    { name: 'page', type: 'Query', schema: z.number().int().optional() },
    { name: 'limit', type: 'Query', schema: z.number().int().optional() },
    { name: 'order_by', type: 'Query', schema: z.string().optional() },
    { name: 'filter', type: 'Query', schema: z.string().optional() },
  ],
  response: z.object({ hits: searchContentResponseSchema }),
});
